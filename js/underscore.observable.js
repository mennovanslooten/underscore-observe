var addObservableMixin = function (und) {
    ...
    function ObservableArray(subject) {
        ...
        function callGenericSubscribers() {
            und.each(_handlers.generic, function (f) {
        ...
        function callCreateSubscribers(new_item, item_index) {
            und.each(_handlers.create, function (f) {
        ...
        function callUpdateSubscribers(new_item, old_item, item_index) {
            und.each(_handlers.update, function (f) {
        ...
        function callDeleteSubscribers(deleted_item, item_index) {
            und.each(_handlers['delete'], function (f) {
        ...
        function detectChanges() {
            ...
            if (old_length != new_length || JSON.stringify(_old_subject) != JSON.stringify(subject)) {
                ...
                for (var i = max; i >= 0; i--) {
                    ...
                    } else if (!und.isEqual(new_item, old_item)) {
        ...
        subject.reverse = function () {
            ...
            und.each(this, callCreateSubscribers);
            reset();
            return result;
        };
        ...
        subject.splice = function (i, length /*, insert */) {
            ...
            und.each(insert, function (new_item, k) {
    ...
    und.mixin({
        observe:function (subject, type, f) {
            if (!und.isArray(subject)) {
                throw "subject should be a array";
            }

            if (und.isFunction(type)) {
                f = type;
                type = 'generic';
            }

            var index = und.indexOf(_subjects, subject);
            if (index == -1) {
...
if (typeof module !== "undefined" && module !== null) {
    module.exports = addObservableMixin;
} else {
    addObservableMixin(_);
}
